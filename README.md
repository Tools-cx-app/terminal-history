...
`recall`、`candidates` 等内部命令会在写入入口自动忽略。

默认键绑定会直接替换各 Shell 原生的 history 搜索和上下翻页绑定：

| 按键 | 行为 |
| --- | --- |
| `Ctrl-R` | 使用内置 Ratatui 界面交互搜索当前目录的历史 |
| `↑` / `↓`（Bash、Zsh、Fish） | 根据当前命令行前缀浏览当前目录的历史 |
| `↑` / `↓`（Nushell） | 打开并浏览当前目录、当前命令行前缀的历史菜单 |

Nushell 首次按 `↑` 会打开 Reedline 历史菜单；继续按 `↑` / `↓` 选择候选，`Enter`
接受候选并返回可编辑的命令行，`Esc` 取消。候选查询期间 Reedline 会保持 raw mode，
因此快速连按或长按方向键不会把 `^[[A` / `^[[B` 等控制序列回显到终端。

Nushell 0.112 及以上的行内历史提示也会替换为当前目录的数据库历史。
